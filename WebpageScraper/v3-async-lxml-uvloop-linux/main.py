...
"""
#TODO: cache the results / save the result of each stage locally
...
async def parse_row(row, table_headers=None, keys_of_interest=None, select_all=False):
    ...
    for key, value in zip(table_headers, cells):
        ...
        if key in keys_of_interest or select_all and len(value.text.strip()) > 0:
            ...
            to_save = await utils.parse_table_cell(value) if value.text.strip() != settings.NO_VALUE else to_save  #
            if to_save == None:
                continue
...
async def main() -> None:
    ...
    # Note: When running with WSL, I think you must create soft link to the images folder,
    #       Otherwise it won't find the images sources
    #await utils.create_html_output(animals_by_collateral_adjectives)
