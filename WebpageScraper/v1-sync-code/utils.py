...
def retrieve_content(uri: str,session) -> bytes:
    """
    ...
    try:
        response = session.get(uri) #TODO: change here to async
    except requests.exceptions.RequestException as e:
        MAIN_LOGGER.exception(f'Failed to get animal image data: {e} (uri={uri})')
    else:
        # No exception was raised, check the response
        if not response.ok:
            MAIN_LOGGER.warning(f'Bad response code: {response.status_code} (uri={uri})')
        else:
            # MODULE_LOGGER.debug(f'Got an affirmative response: {response.status_code} (uri={uri})')
